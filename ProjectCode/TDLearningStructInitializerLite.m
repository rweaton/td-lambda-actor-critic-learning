function OutputStruct = TDLearningStructInitializerLite(...
...
%     Debugged: 6/1/2009
    ...
    OutputStruct.StepsBetweenSaves = StepsBetweenSaves;
    
    OutputStruct.PreviousV_s = zeros([nStates,1]);
    OutputStruct.V_sRecord = NaN*ones([nStates, ...
        floor(nStepsPerEpisode/StepsBetweenSaves)]);
    
    %OutputStruct.V_s = rand([nStates,nStepsPerEpisode]);
    %OutputStruct.V_s(:,end) = 0;
    
    OutputStruct.PreviousPref_sa = zeros([nStates,1,nActions]);
    OutputStruct.Pref_saRecord = NaN*ones([nStates, ...
        floor(nStepsPerEpisode/StepsBetweenSaves),nActions]);
